...
from open_webui.utils.auth import (
    get_admin_functions_read_access,
    get_admin_functions_write_access,
    get_verified_user,
)
...
async def get_functions(user=Depends(get_verified_user)):  # noqa
...
async def get_functions(user=Depends(get_admin_functions_read_access)):  # noqa
    return Functions.get_functions()
...
async def create_new_function(
    ...
    user=Depends(get_admin_functions_write_access),
):
    ...
    if Functions.name_exists(form_data.name):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def get_function_by_id(id: str, user=Depends(get_admin_functions_read_access)):
...
async def toggle_function_by_id(
    id: str, user=Depends(get_admin_functions_write_access)
):
    function = Functions.get_function_by_id(id)
    if function:
...
async def toggle_global_by_id(id: str, user=Depends(get_admin_functions_write_access)):
    function = Functions.get_function_by_id(id)
    if function:
        function = Functions.update_function_by_id(
...
async def update_function_by_id(
    ...
    if Functions.name_exists(form_data.name, exclude_id=id):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=ERROR_MESSAGES.NAME_TAKEN,
        )
...
async def delete_function_by_id(
    request: Request, id: str, user=Depends(get_admin_functions_write_access)
):
...
async def get_function_valves_by_id(
    id: str, user=Depends(get_admin_functions_read_access)
):
    function = Functions.get_function_by_id(id)
    if function:
        try:
...
async def get_function_valves_spec_by_id(
    request: Request, id: str, user=Depends(get_admin_functions_read_access)
):
    function = Functions.get_function_by_id(id)
    if function:
