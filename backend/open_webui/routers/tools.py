...
router = APIRouter(dependencies=[Depends(require_feature("tools"))])
...
async def get_tools(request: Request, user=Depends(get_verified_user)):
    if user.role != "admin" and not has_permission_min_level(
        user.id, "workspace.tools", "read", request.app.state.config.USER_PERMISSIONS
    ):
        raise HTTPException(
...
async def get_tool_list(request: Request, user=Depends(get_verified_user)):
    if user.role != "admin" and not has_permission_min_level(
        user.id,
        "workspace.tools",
        "read",
...
async def export_tools(user=Depends(get_admin_functions_read_access)):
    tools = Tools.get_tools()
    return tools
...
async def create_new_tools(
    ...
    if user.role != "admin" and not has_permission_min_level(
        user.id, "workspace.tools", "write", request.app.state.config.USER_PERMISSIONS
    ):
        raise HTTPException(
    ...
    if Tools.name_exists(form_data.name):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def update_tools_by_id(
    ...
    if Tools.name_exists(form_data.name, exclude_id=id):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=ERROR_MESSAGES.NAME_TAKEN,
        )
...
async def get_tools_valves_by_id(id: str, user=Depends(get_verified_user)):
    ...
    if (
        tools.user_id != user.id
        and not has_access(user.id, "read", tools.access_control)
        and user.role != "admin"
    ):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
