...
@router.get("/config")
async def get_config(request: Request, user=Depends(get_admin_evaluations_read_access)):
    return {
...
async def update_config(
    ...
    AuditLogger.log_settings_change("evaluations", after_data=form_data.model_dump())
    return {
...
async def get_all_feedbacks(user=Depends(get_admin_evaluations_read_access)):
...
async def delete_all_feedbacks(user=Depends(get_admin_evaluations_write_access)):
    success = Feedbacks.delete_all_feedbacks()
    return success


@router.get("/feedbacks/all/export", response_model=list[FeedbackModel])
async def get_all_feedbacks(user=Depends(get_admin_evaluations_read_access)):  # noqa
    feedbacks = Feedbacks.get_all_feedbacks()
    return [
        FeedbackModel(
