...
async def update_config(
    ...
    request.app.state.config.OLLAMA_API_CONFIGS = resolve_config_dict(
        form_data.OLLAMA_API_CONFIGS,
    ...
    AuditLogger.log_settings_change(
        "connections/ollama", after_data=form_data.model_dump()
    )
    return mask_config_dict(
        {
            "ENABLE_OLLAMA_API": request.app.state.config.ENABLE_OLLAMA_API,
...
async def generate_chat_completion(
    ...
    if model_info:
        ...
        payload = apply_model_system_prompt_to_body(
            params or {}, payload, metadata, user
        )
    ...
    # === Unified Agent: Ollama 에이전트도 UnifiedAgent 라우팅 ===
    if not (metadata or {}).get("task", None) and form_data.stream and model_info:
        ...
        # Ollama payload → OpenAI 호환 변환
        if "options" in payload_for_runner:
            opts = payload_for_runner.pop("options", {})
            if "temperature" in opts:
                payload_for_runner["temperature"] = opts["temperature"]
            if "top_p" in opts:
                payload_for_runner["top_p"] = opts["top_p"]
            if "num_predict" in opts:
                payload_for_runner["max_tokens"] = opts["num_predict"]
        payload_for_runner["stream"] = True
        ...
        # UnifiedAgent 응답은 이미 OpenAI 형식 — 변환 건너뛰기 마커
        if hasattr(result, "headers"):
            result.headers["x-openai-format"] = "true"
        return result
