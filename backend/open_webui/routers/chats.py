...
async def get_session_user_chat_list(
    ...
    project_chat_ids = Projects.get_project_chat_ids_by_user_id(user.id)
    exclude = project_chat_ids if project_chat_ids else None
...
async def search_user_chats(
    ...
    chat_list = [
        ...
        for chat in Chats.get_chats_by_user_id_and_search_text(
            user.id, text, skip=skip, limit=limit, exclude_chat_ids=exclude
        )
    ]
...
async def get_user_pinned_chats(user=Depends(get_verified_user)):
    ...
    return [
        ChatResponse(**chat.model_dump())
        for chat in Chats.get_pinned_chats_by_user_id(user.id, exclude_chat_ids=exclude)
    ]
...
async def get_chat_by_id(id: str, user=Depends(get_verified_user)):
    ...
    # 소유자 아님 → schedule 결과 chat인지 확인하고, 해당 schedule의 read 권한자에게는 허용
    chat = Chats.get_chat_by_id(id)
    if chat:
        schedule_id = (chat.meta or {}).get("schedule_id")
        if schedule_id:
...
async def update_chat_by_id(
    ...
    if chat:
        ...
        if "title" not in form_data.chat:
            updated_chat["title"] = chat.title
...
async def update_chat_title_by_id(
    id: str, form_data: ChatTitleForm, user=Depends(get_verified_user)
):
    chat = Chats.get_chat_by_id_and_user_id(id, user.id)
    if chat:
        chat = Chats.update_chat_title_by_id(
            id, form_data.title, update_timestamp=False
        )
        return ChatResponse(**chat.model_dump())
    else:
        raise HTTPException(
