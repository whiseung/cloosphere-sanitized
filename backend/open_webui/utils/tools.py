...
def get_tools(
    ...
    # Pre-fetch user-accessible tool IDs for permission checks
    user_tools = Tools.get_tools_by_user_id(user.id, "read")
    accessible_tool_ids = {t.id for t in user_tools}

    for tool_id in tool_ids:
        # Server tools bypass local permission check
        if tool_id.startswith("server:"):
            tool = None
        else:
            # Check user access before loading
            if tool_id not in accessible_tool_ids:
                log.warning(f"User {user.id} has no access to tool {tool_id}, skipping")
                continue
