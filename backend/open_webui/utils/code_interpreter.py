...
class JupyterCodeExecuter:
    ...
    async def execute_in_jupyter(self, ws) -> None:
        result = await execute_in_kernel(ws, self.code, self.timeout)
        self.result = result


async def execute_in_kernel(ws, code: str, timeout: int = 60) -> ResultModel:
    """Execute code in a Jupyter kernel via an open WebSocket connection.

    Shared by JupyterCodeExecuter (ephemeral kernels) and
    JupyterSessionManager (persistent project kernels).
    """
