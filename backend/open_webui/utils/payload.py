...
def apply_model_system_prompt_to_body(
    ...
    # Extract system language from metadata variables
    system_language = ""
    if metadata:
        variables = metadata.get("variables") or {}
        system_language = variables.get("{{USER_LANGUAGE}}", "")

    if not system:
        # No model system prompt — inject system language context as fallback
        if system_language:
            messages = form_data.get("messages", [])
            existing = next((m for m in messages if m.get("role") == "system"), None)
            language_hint = f"System Language: {system_language}"
            if existing:
                content = existing.get("content", "")
                if "System Language:" not in content:
                    existing["content"] = f"{content}\n\n{language_hint}"
            else:
                form_data["messages"] = add_or_update_system_message(
                    language_hint, messages
                )
        return form_data

    # Check if original template already includes {{USER_LANGUAGE}}
    template_has_language = "{{USER_LANGUAGE}}" in system
    ...
    # Append system language if the original template didn't include {{USER_LANGUAGE}}
    if system_language and not template_has_language:
