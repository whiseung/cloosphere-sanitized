...
async def get_all_models(request, user: UserModel = None):
    ...
    for custom_model in custom_models:
        if custom_model.base_model_id is None:
            # Check if this is an Agent Flow model
            is_agent_flow = False
            ...
            if is_agent_flow and custom_model.is_active:
                # Add Agent Flow model to MODELS
                action_ids = []
                if custom_model.meta:
                    meta = custom_model.meta.model_dump()
    ...
    # Preserve existing cache if new model list is empty (API failure)
    existing_models = getattr(request.app.state, "MODELS", {})
    new_models = {model["id"]: model for model in models}
    if new_models:
        request.app.state.MODELS = new_models
    elif existing_models:
        log.warning(
            "get_all_models() returned empty list — keeping existing cache "
            f"({len(existing_models)} models)"
        )
    else:
