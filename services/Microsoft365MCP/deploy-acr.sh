#!/usr/bin/env bash
# Microsoft 365 MCP 이미지를 Azure Container Registry 에 빌드 + 푸시한다.
# `az acr build` 를 사용 → 빌드를 ACR 가 클라우드에서 수행하므로 로컬 docker 가 불필요하고
# 항상 linux/amd64 로 빌드된다(로컬 아키텍처 무관). 버전 태그 + latest 두 개를 함께 푸시.
#
# 사용 예:
#   ./deploy-acr.sh                 # 기본값 (acrcloosphere / m365-mcp / .env 의 버전)
#   ./deploy-acr.sh 0.115.0         # 버전 지정 (이미지 태그 + Dockerfile build-arg 동시 적용)
#   REGISTRY=otheracr ./deploy-acr.sh
#   VERSION=0.115.0 ./deploy-acr.sh
#
...
IMAGE="${IMAGE:-m365-mcp}"              # 리포지토리 이름
DEFAULT_VERSION="0.114.0"

# 버전 우선순위: 1번째 인자 > $VERSION > .env 의 MS365_MCP_VERSION > 기본값
ENV_VERSION=""
if [ -f .env ]; then
  ENV_VERSION="$(grep -E '^MS365_MCP_VERSION=' .env | cut -d= -f2- | tr -d '[:space:]' || true)"
fi
VERSION="${1:-${VERSION:-${ENV_VERSION:-$DEFAULT_VERSION}}}"

echo "▶ ACR 빌드 + 푸시 (Microsoft 365 MCP)"
...
az acr build \
  ...
  --image "${IMAGE}:${VERSION}" \
  --image "${IMAGE}:latest" \
  --build-arg "MS365_MCP_VERSION=${VERSION}" \
  --file Dockerfile \
  .
