<script lang="ts">
	...
	onMount(async () => {
		...
		if (!raw) {
			async function tryParse(value, attempts = 3, interval = 100) {
				try {
					// Try parsing the value with appropriate breaks setting
					const processedValue = preserveBreaks
						? value.replaceAll(`\n`, `\n<br/>`)
						: value.replaceAll(`\n<br/>`, `<br/>`);
					return marked.parse(processedValue, {
						breaks: preserveBreaks
					});
				} catch (error) {
	...
	$: if (
		...
		if (raw) {
			...
			// Parse markdown with appropriate breaks setting
			const parsedContent = marked.parse(
				preserveBreaks ? value.replaceAll(`\n`, `\n<br/>`) : value.replaceAll(`\n<br/>`, `<br/>`),
				{ breaks: preserveBreaks }
			);
