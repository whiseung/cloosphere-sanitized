		...
		<div class="flex flex-col justify-center -space-y-0.5 px-2.5 w-full">
			...
			<div class=" flex justify-between text-gray-500 text-xs line-clamp-1">
				<div class="flex items-center gap-1">
					...
					{/if}
					{#if item?.file?.meta?.classification?.category}
						{@const cat = item.file.meta.classification.category}
						<span class="text-[10px] px-1 py-0.5 rounded {
							cat === 'PUBLIC' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' :
							cat === 'INTERNAL' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300' :
							cat === 'CONFIDENTIAL' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300' :
							cat === 'RESTRICTED' ? 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300' :
