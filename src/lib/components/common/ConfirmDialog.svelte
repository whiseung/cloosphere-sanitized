	...
	>
		...
		>
			<div class="px-[1.75rem] py-6 flex flex-col">
					...
					<div class="text-sm text-[var(--cloo-text-muted)] flex-1 whitespace-pre-line">
						...
						{#if input}
							<textarea
								...
								class="w-full mt-2 rounded-[var(--cloo-radius-default)] px-4 py-2 text-sm bg-[var(--cloo-bg-surface)] text-[var(--cloo-text-default)] border border-[var(--cloo-border-subtle)] outline-hidden resize-none"
								rows="3"
								required
							/>
						{/if}
				...
				<!-- Use the shared button token set for both actions. -->
