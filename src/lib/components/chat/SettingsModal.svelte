<script lang="ts">
	...
	const searchData: SettingsTab[] = [
		{
			...
			keywords: [
				...
				'defaultsettings',
				'configuration',
				// i18n keys for multilingual search
				'Theme',
				'Language',
				'Notifications',
				'System Prompt',
	...
	const searchSettings = (query: string): string[] => {
		...
		return searchData
			.filter((tab) => {
				const titleText = $i18n.t(tab.title).toLowerCase();
				const keywordTexts = tab.keywords
					.flatMap((keyword) => {
						const titleKey = toTitleKey(keyword);
						return [keyword, titleKey, $i18n.t(keyword), $i18n.t(titleKey)];
					})
				...
				return (
					...
					keywordTexts.some((keyword) => keyword.includes(lowerCaseQuery)) ||
...
<Modal size="xl" bind:show>
	<div class="text-gray-700 dark:text-gray-100">
		...
		<div class="flex flex-col md:flex-row w-full px-4 pt-1 pb-4 md:space-x-4">
			...
			>
				<div class="hidden md:flex w-full -mb-1" id="settings-search">
					<Input
						bind:value={search}
				...
				{#if visibleTabs.length > 0}
					{#each visibleTabs as tabId (tabId)}
						...
						{:else if tabId === 'connections'}
							{#if $user?.role === 'admin' || ($user?.role === 'user' && ($config?.features?.enable_direct_connections || $config?.features?.enable_gmail || $config?.features?.enable_calendar || $config?.features?.enable_drive))}
								<button
									class="px-0.5 py-1 min-w-fit rounded-lg flex-1 md:flex-none flex text-left transition {selectedTab ===
									'connections'
										? ''
