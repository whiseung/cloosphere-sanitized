<script lang="ts">
	...
	let fuse = new Fuse(
		$models
			.filter((model) => !model?.hidden && !model?.info?.meta?.hidden)
			.map((model) => {
				const _item = {
					...model,
	...
	$: filteredItems = command.slice(1)
		...
		: $models.filter((model) => !model?.hidden && !model?.info?.meta?.hidden);
					...
					{#each filteredItems as model, modelIdx}
						...
						>
							<div class="flex font-medium text-black dark:text-gray-100 line-clamp-1">
								<img
									src={model?.info?.meta?.profile_image_url ?? $brandingUrls.favicon}
