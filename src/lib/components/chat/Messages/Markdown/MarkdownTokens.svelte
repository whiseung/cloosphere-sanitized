...
{#each tokens as token, tokenIdx (tokenIdx)}
	...
	{:else if token.type === 'html'}
		{@const html = DOMPurify.sanitize(token.text, {
			ALLOWED_TAGS: ['table', 'thead', 'tbody', 'tr', 'th', 'td', 'video', 'source', 'iframe', 'br', 'div', 'span', 'p', 'strong', 'em', 'ul', 'ol', 'li', 'a', 'img', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'blockquote', 'pre', 'code'],
			ALLOWED_ATTR: ['src', 'href', 'alt', 'title', 'class', 'style', 'width', 'height', 'colspan', 'rowspan', 'border', 'cellpadding', 'cellspacing', 'target', 'rel']
		})}
		...
		{:else if html && (html.includes('<table') || html.includes('<div') || html.includes('<p') || html.includes('<ul') || html.includes('<ol') || html.includes('<br'))}
...
<style>
	...
	:global(.dark .html-content th),
	:global(.dark .html-content td) {
		border-color: #374151;
	}
	:global(.html-content th) {
	...
	:global(.html-content tr:nth-child(even)) {
		background-color: #f9fafb;
	}
	:global(.dark .html-content tr:nth-child(even)) {
