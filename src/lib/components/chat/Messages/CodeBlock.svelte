<script lang="ts">
	...
	$: isPreviewable = ['html', 'svg'].includes(lang?.toLowerCase());
...
<div>
	<div class="relative {className} flex flex-col rounded-lg" dir="ltr">
		...
		{:else}
			...
			>
				<div class="flex items-center gap-0.5 translate-y-[1px]">
					...
					{#if isPreviewable}
						...
						>
							<div>
								{showPreview ? $i18n.t('Hide Preview') : $i18n.t('Preview')}
							</div>
						</button>
					{/if}
			...
			{#if !collapsed && showPreview && isPreviewable}
				<div class="border border-gray-200 dark:border-gray-700 rounded-b-lg overflow-hidden bg-white">
					...
					{:else}
						<iframe
							...
							style="min-height: 200px; height: 400px; resize: vertical;"
