<script lang="ts">
	...
	const getModelType = (item: any): 'agent' | 'flow' | 'model' => {
		if (item.model?.owned_by === 'flow' || item.model?.owned_by === 'agent_flow') return 'flow';
		if (item.model?.preset && item.model?.info?.base_model_id) return 'agent';
		return 'model';
	};

	const modelTypeSortOrder = { agent: 0, flow: 1, model: 2 };
	const modelTypeLabels = { agent: 'Agent', flow: 'Flow', model: 'Model' };

	let badgeMinWidth = 0;

	function measureBadge(node: HTMLElement) {
		const update = () => {
			if (node.offsetWidth > badgeMinWidth) {
				badgeMinWidth = node.offsetWidth;
			}
		};
		update();
		return { update };
	...
	$: filteredItems = (
		searchValue
			? fuse
					...
					.filter((item) => {
						...
						} else if (selectedConnectionType === 'flow') {
							return item.model?.owned_by === 'flow';
						}
					})
			: items
					.filter((item) => {
		...
		.sort(
			(a, b) =>
				modelTypeSortOrder[getModelType(a)] - modelTypeSortOrder[getModelType(b)] ||
	...
	onMount(async () => {
		// Only check Ollama version if Ollama models exist (avoids unnecessary request)
		if (items.some((item) => item.model?.owned_by === 'ollama')) {
...
<!-- Hidden badge width measurement -->
<div class="absolute invisible pointer-events-none" aria-hidden="true">
	{#each Object.values(modelTypeLabels) as label}
		<span
			class="inline-block text-[10px] font-semibold px-1 py-0.5"
			use:measureBadge
		>{label}</span>
	{/each}
</div>
				...
				{#if tags && items.filter((item) => !(item.model?.info?.meta?.hidden ?? false)).length > 0}
					...
					>
						...
						>
							...
							{#if items.find((item) => item.model?.owned_by === 'flow')}
								<button
									class="min-w-fit outline-none p-1.5 {selectedConnectionType === 'flow'
										? ''
										: 'text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'} transition capitalize"
									on:click={() => {
										selectedTag = '';
										selectedConnectionType = 'flow';
									}}
								>
									{$i18n.t('Flows')}
								</button>
					...
					>
						<div class="flex flex-col">
							...
							<div class="flex items-center gap-2">
									...
									<div class="line-clamp-1">
										<div class="flex items-center min-w-fit">
											<Tooltip
												...
												placement="top-start"
											>
												{#if getModelType(item) === 'agent'}
													<span class="shrink-0 text-center text-[10px] font-semibold py-0.5 rounded mr-1.5 bg-blue-500/15 text-blue-600 dark:text-blue-400" style="min-width: {badgeMinWidth}px">{modelTypeLabels.agent}</span>
												{:else if getModelType(item) === 'flow'}
													<span class="shrink-0 text-center text-[10px] font-semibold py-0.5 rounded mr-1.5 bg-purple-500/15 text-purple-600 dark:text-purple-400" style="min-width: {badgeMinWidth}px">{modelTypeLabels.flow}</span>
												{:else}
													<span class="shrink-0 text-center text-[10px] font-semibold py-0.5 rounded mr-1.5 bg-gray-500/15 text-gray-600 dark:text-gray-400" style="min-width: {badgeMinWidth}px">{modelTypeLabels.model}</span>
												{/if}

												<img
													src={item.model?.info?.meta?.profile_image_url ?? $brandingUrls.favicon}
													alt="Model"
								...
								{:else if item.model.owned_by === 'flow'}
									<Tooltip content={$i18n.t('Flow')}>
										<div class="translate-y-[1px]">
											<svg
												xmlns="http://www.w3.org/2000/svg"
												viewBox="0 0 16 16"
											...
											>
												<path
													fill-rule="evenodd"
													d="M8 1a.75.75 0 0 1 .75.75v1.5h2.75a.75.75 0 0 1 0 1.5H8.75v3.5h2.75a.75.75 0 0 1 0 1.5H8.75v1.5h2.75a.75.75 0 0 1 0 1.5H8.75v1.5a.75.75 0 0 1-1.5 0v-1.5H4.5a.75.75 0 0 1 0-1.5h2.75v-1.5H4.5a.75.75 0 0 1 0-1.5h2.75v-3.5H4.5a.75.75 0 0 1 0-1.5h2.75v-1.5A.75.75 0 0 1 8 1Z"
													clip-rule="evenodd"
												/>
											</svg>
										</div>
