<script lang="ts">
	...
	export let showReindex: boolean = false;
	export let showCopy: boolean = false;
	export let showClone: boolean = false;
		...
		>
			{#if showReindex}
				...
				>
					...
					<div class="flex items-center">{$i18n.t('Rebuild search index')}</div>
				</DropdownMenu.Item>
			{/if}
			{#if showCopy}
				<DropdownMenu.Item
				...
				>
					...
					<div class="flex items-center">{$i18n.t('Copy to scope')}</div>
				</DropdownMenu.Item>
			{/if}
			{#if showClone}
				<DropdownMenu.Item
				...
				>
					<Square2Stack className="size-4" strokeWidth="2" />
