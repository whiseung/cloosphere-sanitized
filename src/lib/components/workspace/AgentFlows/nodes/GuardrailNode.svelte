...
<div
	class="flow-node guardrail-node px-4 py-3 rounded-xl border-2 min-w-[180px]
...
>
	<Handle
		...
		class="!w-3 !h-3 !bg-rose-500 !border-2 !border-white dark:!border-gray-800"
	/>
	...
	<div class="flex items-center gap-2">
		...
		<div class="flex-1 min-w-0">
			<div class="text-xs font-semibold text-rose-700 dark:text-rose-300">
				{$i18n?.t?.('Guardrail') || 'Guardrail'}
			</div>
			<div class="text-[10px] text-rose-600 dark:text-rose-400 truncate">
				{data?.label || $i18n?.t?.('Safety Filter') || 'Safety Filter'}
			</div>
		</div>
	</div>
	...
	<!-- Always show both pass and block outputs -->
	<Handle type="source" position={Position.Bottom} id="pass" style="left: 25%;" class="!w-3 !h-3 !bg-green-500 !border-2 !border-white dark:!border-gray-800" />
	<Handle type="source" position={Position.Bottom} id="block" style="left: 75%;" class="!w-3 !h-3 !bg-red-500 !border-2 !border-white dark:!border-gray-800" />
</div>
