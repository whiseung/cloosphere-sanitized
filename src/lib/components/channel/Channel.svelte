<script lang="ts">
	...
	import { channels, chatId, showSidebar, socket, user, WEBUI_NAME } from '$lib/stores';
	...
	// 사이드바에서 채널 이름 변경 시 현재 페이지에 반영
	$: if ($channels && channel) {
		const updated = $channels.find((c) => c.id === id);
		if (updated && updated.name !== channel.name) {
			channel = { ...channel, name: updated.name };
		}
	}
