<script lang="ts">
	...
	let usageDetail: ModelUsageDetail | null = null;

	const usageItemLabel = (item: ModelUsageItem): string => item.name || item.id;

	const usageItemCaption = (item: ModelUsageItem): string => {
		if (item.kind === 'auto_eval_judge') {
			return $i18n.t('Uses this model as auto-eval judge');
		}
		const parts: string[] = [];
		if (item.direct) parts.push($i18n.t('Direct'));
		if (item.via && item.via.length > 0) {
			const names = item.via.map((v) => v.name || v.id);
			const head = names.slice(0, 2).join(', ');
			const extra = names.length - 2;
			const more = extra > 0 ? ` ${$i18n.t('and {{count}} more', { count: extra })}` : '';
			parts.push($i18n.t('via {{agents}}', { agents: head + more }));
		}
...
- 포맷 지침 본문만 출력하라`;
	...
	onMount(async () => {
		...
		if (model) {
			...
			if (id) {
				getModelUsagesDetail(localStorage.token, id)
					.then((d) => (usageDetail = d))
					.catch(() => (usageDetail = null));
			}
		...
		// grounding — binary(on/off). 명시값은 존중하고, 미설정/legacy 는 백엔드와
		// 동일하게 컨텍스트 기본(에이전트 preset=on / 기본 모델=off) 적용.
...
{#if loaded}
	...
	{#if !edit || (edit && model)}
		...
		>
			<div class="model-editor__shell">
				<div class="model-editor__header">
					<div class="model-editor__title-wrap">
						<div class="flex items-center gap-2">
							...
							{/if}
							<h1 class="model-editor__title">
								{$i18n.t('Edit Model')}
							</h1>
						</div>
					</div>

					<div class="model-editor__header-actions">
				...
				<div class="model-editor__basic-grid">
					<div class="model-editor__basic-fields">
						<Input
							bind:value={name}
							label={$i18n.t('Model Name')}
							caption={$i18n.t('Add a clear, user-facing name for this model.')}
					...
					<div class="model-editor__image-panel">
						<LabelBase label={$i18n.t('Model Image')} size="md" />

						<div class="model-editor__image-stack">
							<button
								class="model-editor__image-button"
								type="button"
								on:click={() => {
							...
							>
								<img
									...
									alt={$i18n.t('Model Image')}
									class="model-editor__image-preview"
								/>
							</button>

							<div class="model-editor__image-actions">
								<button
									class="model-editor__secondary-button"
									type="button"
									on:click={() => {
										filesInputElement.click();
									}}
								...
								<button
									class="model-editor__inline-link"
									type="button"
									on:click={() => {
				...
				<!-- <div class="model-editor__section">
					<div class="model-editor__section-heading is-inline">
						<div>
					...
					{#if enableDescription}
						<Textarea
							...
							size="md"
							autoResize={false}
							containerClassName="model-editor__textarea-surface"
						/>
					{/if}
				</div> -->
				...
				<!-- {#if preset}
					<div class="model-editor__section">
						<div class="model-editor__field-panel model-editor__field-panel--plain">
							<LabelBase
							...
							>
								<svelte:fragment slot="right">
									<div class="model-editor__selector-wrap">
										<Selector
				...
				<div class="model-editor__section">
					...
					<div class="model-editor__stack">
						<Textarea
							...
							size="md"
							autoResize={false}
							containerClassName="model-editor__textarea-surface model-editor__textarea-tall"
						>
							<svelte:fragment slot="right">
								<div class="model-editor__prompt-actions">
									<button
										class="model-editor__ai-button"
									...
									<div class="model-editor__selector-wrap is-compact">
										<Selector
						...
						<div class="model-editor__section">
							...
							{#if showAdvanced}
								<div class="model-editor__contained-panel">
									<AdvancedParams
										admin={true}
										bind:params
										on:change={() => {
				...
				<!-- [ModelEditor] 프롬프트 제안 섹션 — admin 모델 편집에서는 미사용
				<div class="model-editor__divider"></div>
					...
					{#if info?.meta?.suggestion_prompts}
						<div class="model-editor__contained-panel">
							{#if info.meta.suggestion_prompts.length > 0}
								<div class="model-editor__suggestions">
									{#each info.meta.suggestion_prompts as prompt, promptIdx}
										<div class="model-editor__suggestion-row">
											<input
												class="model-editor__suggestion-input"
												placeholder={$i18n.t('Write a prompt suggestion (e.g. Who are you?)')}
												bind:value={prompt.content}
											/>

											<button
												class="model-editor__icon-button"
												type="button"
												on:click={() => {
							...
							{:else}
								<div class="model-editor__empty-state">{$i18n.t('No suggestion prompts')}</div>
							{/if}
						</div>
					{/if}
				</div>
				...
				<div class="model-editor__section model-editor__stack">
					<!-- [ModelEditor] 지식그래프·지식·DbSphere·도구·용어집 — 에이전트 전용, admin 모델 편집에서는 미사용
					<KnowledgeGraph
					...
					/>
					-->
					<!-- 가드레일은 도구와 달리 일반(비에이전트) 모델에서도 런타임에 적용되므로 노출 -->
					<GuardrailsSection bind:selectedGuardrails={guardrailsList} {canWrite} />
					<!-- [ModelEditor] 자동평가·응답형식 — admin 모델 편집에서는 미사용
					...
					{#if usageDetail}
						...
						<!-- build-marker:pr180-usage-section -->
						<div class="model-editor__section">
							<div class="model-editor__section-heading">
								<h2>{$i18n.t('Used by')}</h2>
								<p>
									{$i18n.t(
										'Agents, flows, and evaluations that depend on this model. Disabling or deleting this model affects every item listed below.'
									)}
								</p>
							</div>

							<div class="model-usages-grid">
								{#each [{ key: 'agents', title: $i18n.t('Agents'), items: usageDetail.agents }, { key: 'flows', title: $i18n.t('Flows'), items: usageDetail.flows }, { key: 'evaluations', title: $i18n.t('Evaluations'), items: usageDetail.evaluations }] as col (col.key)}
									<div class="model-usages-col">
										<div class="model-usages-col__head">
											<span class="model-usages-col__title">{col.title}</span>
											<span class="model-usages-col__count">{col.items.length}</span>
										</div>
										{#if col.items.length === 0}
											<div class="model-usages-empty">
												{$i18n.t('None')}
											</div>
										{:else}
											<ul class="model-usages-list">
												{#each col.items as item (item.id)}
													<li class="model-usages-item">
														<span class="model-usages-item__name">{usageItemLabel(item)}</span>
														{#if usageItemCaption(item)}
															<span class="model-usages-item__id">{usageItemCaption(item)}</span>
														{/if}
													</li>
												{/each}
											</ul>
				...
				<!-- <div class="model-editor__section">
					...
					{#if showPreview}
						<div class="model-editor__contained-panel">
							<textarea
								class="model-editor__json-preview"
								rows="10"
...
<style>
	...
	.model-editor__primary-button,
	.model-editor__secondary-button,
	.model-editor__ai-button,
	.model-editor__icon-button {
		display: inline-flex;
		align-items: center;
		justify-content: center;
		gap: 0.375rem;
	...
	.model-editor__primary-button:hover:not(:disabled) {
		background: var(--cloo-color-primary-hover);
	}

	.model-editor__primary-button:disabled,
	.model-editor__secondary-button:disabled,
	.model-editor__ai-button:disabled {
		opacity: 0.5;
		cursor: not-allowed;
	}
	...
	.model-editor__secondary-button:hover:not(:disabled),
	.model-editor__icon-button:hover:not(:disabled) {
	...
	.model-editor__image-preview {
		width: 100%;
		height: 100%;
		object-fit: cover;
	}

	.model-editor__image-actions {
		display: flex;
		flex-direction: column;
		align-items: center;
		gap: 0.375rem;
	...
	.model-editor__section {
		display: flex;
		flex-direction: column;
		gap: var(--cloo-space-2);
	}

	.model-editor__section-heading {
		display: flex;
		flex-direction: column;
		gap: var(--cloo-space-1);
	}

	.model-editor__section-heading.is-inline {
		flex-direction: row;
	...
	.model-editor__section-heading h2 {
		margin: 0;
		color: var(--cloo-text-primary);
		font-size: 1rem;
		line-height: 1.5rem;
	...
	.model-editor__section-heading p {
		margin: 0;
		color: var(--cloo-text-muted);
		font-size: 0.75rem;
		line-height: 1rem;
	...
	.model-editor__field-panel,
	.model-editor__contained-panel {
	...
	.model-usages-col__title {
		font-size: 0.8125rem;
		font-weight: 600;
		color: var(--cloo-text-primary);
	}

	.model-usages-col__count {
		font-size: 0.8125rem;
		font-weight: 600;
	...
	.model-usages-item:hover {
		background: var(--cloo-bg-neutral-hovered);
	}

	.model-usages-item__name {
		font-size: 0.8125rem;
		font-weight: 500;
	...
	.model-editor__field-panel--plain,
	.model-editor__contained-panel--plain {
		padding: 0;
		border: 0;
		border-radius: 0;
		background: transparent;
	...
	.model-editor__field-panel--plain :global(.cloo-label-base__left) {
		flex: 0 0 33%;
	}

	.model-editor__field-panel--plain :global(.cloo-label-base__right) {
		flex: 0 0 25%;
		justify-content: flex-end;
	}

	.model-editor__selector-wrap {
		width: 100%;
	}

	.model-editor__selector-wrap.is-compact {
		flex-shrink: 0;
		width: min(100%, 208px);
	}

	:global(.model-editor__textarea-surface) {
		border-color: var(--cloo-border-default);
		background: var(--cloo-bg-default);
	}

	:global(.model-editor__textarea-tall .cloo-textarea__native) {
		min-height: 6rem;
	}

	.model-editor__prompt-actions {
		display: inline-flex;
		align-items: center;
		gap: var(--cloo-space-2);
		flex-wrap: nowrap;
	...
	.model-editor__prompt-actions .model-editor__selector-wrap.is-compact {
		flex: 0 0 220px;
		width: 220px;
	}

	.model-editor__suggestions {
		display: flex;
		flex-direction: column;
		gap: var(--cloo-space-2);
	}

	.model-editor__suggestion-row {
		display: flex;
		align-items: center;
		gap: var(--cloo-space-2);
	...
	.model-editor__suggestion-input {
		flex: 1 1 auto;
		min-width: 0;
		padding: 0.5rem 0;
		border: 0;
