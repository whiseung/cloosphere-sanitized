<script>
	...
	import { WEBUI_NAME, config, user, userPermissions, showSidebar, knowledge } from '$lib/stores';
	...
	import { getOrganizations, getOrganizationalUnits } from '$lib/apis/organizations';
	...
	const loadOrgUnits = async () => {
		try {
			...
			const allUnits = await Promise.all(
				orgs.map((org) => getOrganizationalUnits(localStorage.token, org.id).catch(() => []))
			);
		...
		{:else}
			<div>
				<div class="flex items-center gap-3 text-xs uppercase px-1 font-bold">
					<div class="flex-[2]">{$i18n.t('Group')}</div>
					<div class="flex-1">{$i18n.t('Organizations')}</div>
					<div class="flex-1">{$i18n.t('Users')}</div>
					<div class="w-8"></div>
				</div>
				...
				{#each filteredGroups as group}
					<div class="my-2">
						<GroupItem {group} {users} {groups} {orgUnits} {setGroups} />
					</div>
				{/each}
			</div>
		{/if}
