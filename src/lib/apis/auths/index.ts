...
export const setSuperAdmin = async (token: string, email: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/auths/admin/sa`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
			authorization: `Bearer ${token}`
...
export type UsageLimitConfig = {
	ENABLE_USAGE_LIMIT?: boolean | null;
	USAGE_LIMIT_DEFAULT_DAILY_TOKENS?: number | null;
	USAGE_LIMIT_EXCEED_ACTION?: string | null;
	USAGE_LIMIT_PER_MODEL?: Record<string, number> | null;
};

export const getUsageLimitConfig = async (token: string): Promise<UsageLimitConfig> => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/auths/admin/config/usage-limit`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			authorization: `Bearer ${token}`
...
export const updateUsageLimitConfig = async (
	token: string,
	body: UsageLimitConfig
