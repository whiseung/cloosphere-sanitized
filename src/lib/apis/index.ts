...
export const generateText = async (token: string, prompt: string, model: string = '') => {
	let error = null;
	...
	// Gemini/Vertex AI may return content as array of blocks
		...
		if (jsonStartIndex !== -1 && jsonEndIndex !== -1) {
			...
			if (parsed && parsed.follow_ups) {
				return Array.isArray(parsed.follow_ups)
					? parsed.follow_ups.filter((q: unknown) => typeof q === 'string' && q.trim().length > 0)
					: [];
			}
			return [];
		}
...
export const getChangelog = async (lang: string = 'en') => {
	let error = null;

	const res = await fetch(`${WEBUI_BASE_URL}/api/changelog?lang=${lang}`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json'
		}
