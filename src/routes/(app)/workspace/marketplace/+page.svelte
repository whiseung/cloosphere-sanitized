<script lang="ts">
	...
	import { WEBUI_NAME, user, activeWorkspaceFilter } from '$lib/stores';
	...
	// 마켓플레이스가 만든 tool_connection 만 노출 (meta.source==='marketplace').
	const isMarketplaceItem = (it: any): boolean => it?.meta?.source === 'marketplace';
	...
	const serviceLabel = (it: any): string =>
		SERVICE_LABELS[it?.meta?.service_id] ?? $i18n.t('Connection');

	// Check permission
	$: if (
		$user &&
		$user.role !== 'admin' &&
		!hasPermission($user?.permissions?.workspace?.marketplace)
	) {
		goto('/workspace');
	}
	...
	$: if (fuse) {
		let result = query ? fuse.search(query).map((e) => e.item) : items;
	...
	const loadItems = async () => {
		const all =
			(await getToolConnections(localStorage.token).catch((e) => {
				console.error(e);
				return [];
			})) ?? [];
		return all.filter(isMarketplaceItem);
	};
	...
	const deleteHandler = async (item) => {
		...
		if (res) {
			...
			toast.success($i18n.t('Connection deleted successfully.'));
		}
	};
	...
	onMount(async () => {
		// 다른 워크스페이스 페이지에서 넘어온 tag: 필터가 남아있으면 초기화
		if (typeof $activeWorkspaceFilter === 'string' && $activeWorkspaceFilter.startsWith('tag:')) {
			activeWorkspaceFilter.set('all');
		}
...
{#if loaded}
	...
	<div class="flex items-center justify-between gap-[var(--cloo-space-2)] mt-2 mb-4">
		...
		<div class="flex items-center gap-[var(--cloo-space-2)]">
			<div class="w-[300px]">
				<Input bind:value={query} placeholder={$i18n.t('Search connections')} type="search" size="md">
			...
			{#if $user?.role === 'admin' || $user?.permissions?.workspace?.marketplace === 'write'}
				<div class="flex items-center gap-[var(--cloo-space-2)]">
					<Button
						kind="filled"
						size="md"
						on:click={() => {
							goto('/workspace/marketplace/create');
						}}
					>
						<svelte:fragment slot="prefix">
	...
	{#if filteredItems.length === 0}
		<div class="flex flex-col items-center justify-center h-64 text-gray-500 dark:text-gray-400">
			...
			<div class="text-xl font-medium mb-2">{$i18n.t('No connections yet')}</div>
			<div class="text-sm">{$i18n.t('Create a marketplace connection to get started.')}</div>
		</div>
	{:else}
		<div class="mb-5 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
			{#each filteredItems as item}
				<WorkspaceCard
					...
					on:click={() => {
						goto(`/workspace/marketplace/${item.id}`);
					}}
				>
					<svelte:fragment slot="badge">
						<Badge type="info" content={serviceLabel(item)} />
						{#if getShareTag(item)}
	...
	<div class=" text-gray-500 text-xs mt-1 mb-2">
		ⓘ {$i18n.t('Connect Cloosphere first-party services that run as separate containers.')}
	</div>
{:else}
